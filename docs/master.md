...
- /admin/ai-usage: 管理者向けAI使用量・コスト監視
...
- `/admin/ai-usage`
  - 管理者向けAI使用量・コスト監視ページ
  - `ai_usage_logs` を元に、最新ログと集計を表示する
  - 総件数、成功件数、エラー件数、総トークン、今日のトークン、直近7日のトークン、推定コスト、機能別集計、最新ログ一覧を確認できる
  - `document_type_detection` などのAI機能ごとの利用量を確認できる
  - 初期表示は最新100件ベース
  - 管理者判定は `/admin/errors` と同じ `ADMIN_EMAIL` 固定判定
  - 今後の拡張候補: 期間フィルタ、ユーザー別集計、コスト単価計算、異常増加アラート
