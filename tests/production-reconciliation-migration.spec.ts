...
test("pins both the Production inventory and the repository fresh-chain shape", () => {
  ...
  expect(migration).toContain("unexpected moddatetime function acl");
  expect(migration).toContain("unknown autopdf function grant principals");
});

test("recognizes only the exact trusted legacy ACL on public.moddatetime()", () => {
  const migration = sql(MIGRATION);
  const fixture = sql(FIXTURE);
  const aclCheck = migration.slice(
    migration.indexOf("-- supabase owns the extension function"),
    migration.indexOf("if not moddatetime_acl_valid then"),
  );
  ...
  expect(aclCheck).toContain("function_owner = 'supabase_admin'");
  expect(aclCheck).toContain("and count(*) = 6");
  for (const principal of [
    ...
    expect(aclCheck).toContain(`= '${principal}'`);
  }
  expect(aclCheck).toContain("bool_and(grantor.rolname = 'supabase_admin')");
  expect(aclCheck).toContain(
    "acl.privilege_type = 'execute' and not acl.is_grantable",
  );
  expect(migration).toContain("unexpected moddatetime acl postcondition");
  expect(migration).not.toContain(
    "revoke all on function public.moddatetime()",
  );

  expect(fixture).toContain("create role supabase_admin login superuser");
