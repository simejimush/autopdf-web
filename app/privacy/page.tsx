    ...
    <main className={styles.page}>
      <div className={styles.container}>
        ...
        <section className={styles.section}>
          ...
          <p className={styles.paragraph}>
            本サービスは、利用者が設定した条件に基づき、Gmailのメールを検索し、PDF生成およびGoogle
            Driveへの保存に必要な範囲でGoogleユーザーデータを取り扱います。AIによる書類種別判定を利用する場合、メールの件名、送信元、本文の一部および添付ファイル名をOpenAI
            APIへ送信する場合があります。これらの情報には、メールの内容によって、氏名、会社名、住所、金額、取引内容などの個人情報または機密情報が含まれる場合があります。添付ファイルそのものの内容を、AIによる書類種別判定のためにOpenAI
            APIへ送信することはありません。これらの情報は、書類種別判定および本サービスの提供に必要な範囲で利用します。Googleユーザーデータを広告ターゲティングに使用せず、当サービス独自のAIモデル学習目的にも使用しません。AIによる書類種別判定を利用したくない場合は、AIを使用しないファイル名形式を選択できます。この場合、AI書類種別判定のためにメール情報をOpenAI
            APIへ送信しません。
          </p>
        </section>
