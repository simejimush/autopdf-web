...
export async function POST(req: Request) {
  ...
  const { provider_token, provider_refresh_token, expires_at, scopes } =
    body ?? {};
  ...
  if (!provider_refresh_token) {
    return NextResponse.json(
      {
        ok: false,
        step: "need_refresh_token",
        message: "provider_refresh_token is empty",
      },
      { status: 400 },
    );
  }
  ...
  const { error: upsertErr } = await supabase.from("google_connections").upsert(
    {
  ...
  if (upsertErr) {
    return NextResponse.json(
      { ok: false, step: "upsert", error: upsertErr.message },
      { status: 500 },
    );
  }
